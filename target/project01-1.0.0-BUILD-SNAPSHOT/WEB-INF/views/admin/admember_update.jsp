...
<div class="container w-50 shadow rounded border shadow p-5 mt-3">
		  <!-- Nav tabs -->
  <ul class="nav nav-tabs" role="tablist">
    <li class="nav-item">
      <a class="nav-link ${find == 'info' ? 'active':''}" data-bs-toggle="tab" href="#menu1">회원정보 수정</a>
    </li>
    <li class="nav-item">
      <a class="nav-link ${find == 'pwch' ? 'active':''}" data-bs-toggle="tab" href="#menu2">비밀번호 변경</a>
    </li>
  </ul>
  ...
  <div class="tab-content">
	  	<!-- 회원정보 수정 - 패널 -->
	    <div id="menu1" class="container tab-pane fade ${find == 'info' ? 'show active':''}"><br>
...
<%-- 			      <!----------------------------------- 주소 ----------------------------------->
	    ...
	    <!-- 비밀번호 변경 - 패널 -->
	    <div id="menu2" class="container tab-pane fade ${find == 'pwch' ? 'show active':''}"><br>
			<h4 class="text-center">비밀번호 변경</h4><br>
 				<input  class="form-control mb-2" type="text" id="memberNo" value="${dto.no}" readonly/>
				<input  class="form-control mb-2" type="text" id="memberId" value="${dto.id}" readonly/>
				<input  class="form-control mb-2" type="text" id="memberPw" value="${dto.pw}" readonly/>
				<input class="form-control mb-2" type="text" id="pwck" name="pwck" placeholder="현재 비밀번호"/>
			...
			<div class="text-center"><br>
				<button type="button" id="pwChangeBtn" class="btn btn-sm btn-success">비밀번호 변경</button>
				<input type="reset" id="pwChangeReset" class="btn btn-sm btn-warning" value="취소"/>	<!-- form 태그 안에 없어서 안됌, id="pwChangeReset" 자바스크립트로 진행 -->
			    <a href="adminMemberList.do" class="btn btn-sm btn-info">회원 리스트</a>
			</div>
		</div>
  </div>
...
<script>
	...
	let currentPwChk = "";
	// 현재비밀번호
	function adpwCheck(){	// ajax : 비동기 방식
		let dbPw = $("#memberPw").val(); // no로 넘겼기때문에 관리자는 회원-loginDTO 사용불가
		pw = $("#pwck").val();
		$.ajax({		// Post방식
			url: "<c:url value='adpwCheck.do'/>",
			type: "post",
			data: {"pw":pw, "dbPw":dbPw}, 	// json 형태
	...
	$("#pwChangeBtn").on("click", function(){
		// 현재비밀번호 체크 함수호출
		adpwCheck()	// ajax : 비동기방식으로 진행되어 순서대로 진행되어야되기때문에 동기방식처럼 넣어줘야함
...
//		} else if(newPwChk && newConfirmPwChk){
